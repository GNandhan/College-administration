...
<div class="container mb-5">
    <div class="row">
        <div class="section-one col-lg">
            <h2 class="header">Student Login.</h2>
        </div>
    </div>
</div>
<!-- Admin Login Form -->
      ...
      <form class="form-signin">
          <h1 class=" mb-3 text-center">Log in </h1>
            <label for="inputEmail" class="sr-only">Email address</label>
              <input type="email" id="inputEmail" class="form-control" placeholder="Email address" required autofocus name="semail">
            <label for="inputPassword" class="sr-only">Password</label>
              <input type="password" id="inputPassword" class="form-control" placeholder="Password" required name="spass">
...
if(isset($_POST["submit"]))
{
  ...
  $sq=mysqli_query($conn,"select * from student where stud_email='$semail' and stud_pass='$spassword'");
  $check=mysqli_num_rows($sq);
  
if($check>0)
{
  $_SESSION["semail"] = $semail;  
  $_SESSION["spass"] = $spassword;  
